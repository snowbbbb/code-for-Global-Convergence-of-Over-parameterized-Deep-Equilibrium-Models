...
root_train = '.../data/cifar-10-batches-py/cifar01Pic/'
trainset = torchvision.datasets.ImageFolder(root_train, transform=trainTransform)
...
def setup_and_train(epochs, lr, width, Wstd):
    ...
    torch.nn.init.normal_(linear_output.weight, mean=0, std=1)#(1 / math.sqrt(width)))
    ...
    # Train the model
    model.train()
    #fid = open('DEQ_CIFAR_'+str(width)+'_'+str(Wstd)+'_ExperimentResult.txt', 'w')
    fid2 = open('DEQ_CIFAR_' + str(width) + '_' + str(Wstd) +  '_max_singularvalue_ExperimentResult.txt', 'w')
    loss_plot=[]
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        log_interval = 100
        train_acc = 0.0
        for batch, data in enumerate(trainloader, 0):#改这里！！！
...
setup_and_train(epochs, lr, 3000, Wstd)
