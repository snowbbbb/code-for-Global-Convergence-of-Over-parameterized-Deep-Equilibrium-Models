...
trainTransform = transforms.Compose([transforms.Grayscale(1),
                                transforms.ToTensor()])
root_train = '.../data/MNIST/raw/trainRezero01Pic/'
...
def setup_and_train(epochs, lr, width, Wstd):
    ...
    # Train the model
    model.train()
    fid = open('DEQ_mnist_'+str(width)+'_'+str(Wstd)+'_ExperimentResult.txt', 'w')
  #  fid2 = open('DEQ_' + str(width) + '_' + str(Wstd) +  '_max_singularvalue_ExperimentResult.txt', 'w')
    loss_plot=[]
...
setup_and_train(epochs, lr, 19000, Wstd)
